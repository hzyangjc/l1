...
from verl.trainer.ppo.ray_pidtrainer import RayPIDTrainer
import sys
...
def main(config):
    if not ray.is_initialized():
        ...
        # 创建自定义临时目录
        import os
        ray_temp_dir = "/dev/shm/ray_temp"
        os.makedirs(ray_temp_dir, exist_ok=True)
        
        # 设置Ray相关的环境变量
        os.environ["RAY_TMPDIR"] = ray_temp_dir
        os.environ["RAY_STORAGE"] = ray_temp_dir
        ...
        ray.init(
            runtime_env={
                'env_vars': {
                    ...
                    'RAY_STORAGE': ray_temp_dir
                }
            },
            _temp_dir=ray_temp_dir,  # 指定Ray临时文件存储路径
            storage=ray_temp_dir     # 指定对象存储路径
        )
...
def main_task(config):
    ...
    trainer = RayPIDTrainer(config=config,
                            tokenizer=tokenizer,
